...
To determine the linear regression model for this data, we have created a mean population Bachelor's attainment percent (explanatory variable) and a mean daily household income (response variable) for each individual country (averaging over all existing years in the dataset since there are some missing values). Based on the linear regression between a country's average Bachelor's attainment percentage (explanatory) and the country's average daily household income (response), we can model the relationship with:
...
-   $\beta_0 = 1.6366$ is the y-intercept for average daily income for a country

-   $\beta_1 = 0.3846$ represents the average change in mean daily household income (in PPP dollars) for a country for every 1% increase in the average of the country's population percentage with a bachelor's equivalent attainment (denoted $X$).
