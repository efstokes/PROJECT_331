...
To get the results in the table we had to first find three values from the model. Those were the total variance, the variance from the explanatory value, and the variance from the residual. To get these values, we extracted the values straight from the fitted model, and then calculated the sum of the variance in the income, fitted, and residual variables. These, respectively, are the first three values in the table. The final value, the $R^2$, was obtained by dividing the fitted variance by the total variance. This tells us how much of the the variance in the results can be explained by the model. In the context of the data this shows that in 2017 about 43.6% of the differences in mean daily household income between countries can be explained by differences in educational attainment as measured by the proportion of the population with the equivalent of bachelor's degree.
...
There seems to be a approximately normal distribution in $R^2$ values in our predicted income, with a values ranging from 0.05 to 0.35.
