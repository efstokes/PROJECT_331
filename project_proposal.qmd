...
```{r}
set.seed(256)
sims <- map_dfc(1:10, ~ tibble(sim = rand_error(x = prediction, sd = est_sigma)))

colnames(sims) <- str_replace_all(colnames(sims), "\\.\\.\\.", "_")

sims <- bind_cols(tibble(income = country_averages$income), sims)

sim_r_sq <- sims %>%
  select(-income) %>%
  map(~ lm(income ~ ., data = sims)) %>%
  map(glance) %>%
  map_dbl("r.squared")

r2_plot <- ggplot(tibble(R2 = sim_r_sq), aes(x = R2)) +
